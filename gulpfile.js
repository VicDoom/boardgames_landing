...
const runServer = () => {
    ...
    watch('src/pages/developer/styles/**/*.scss', series(buildScssDev, reloadBrowserPage))
    watch('src/pages/designer/styles/**/*.scss', series(buildScssDev, reloadBrowserPage))
...
const buildTwigDev = (done) => {
    src('src/**/*.twig')
        .pipe(twig())
        .pipe(replace('styles/style.scss', 'styles/style.css'))
        .pipe(dest('dist/'))
    done();
}
...
const buildTwigProd = (done) => {
    src('src/**/*.twig')
        ...
        .pipe(replace('styles/style.scss', 'styles/style.min.css'))
        .pipe(dest('./build/'));
    done();
}
...
const buildScssDev = (done) => {
    ...
    src('src/pages/developer/styles/**/*.scss')
      .pipe(sourcemaps.init())
      .pipe(sass().on('error', sass.logError))
      .pipe(sourcemaps.write())
      .pipe(dest('dist/pages/developer/styles'));
    src('src/pages/designer/styles/**/*.scss')
        .pipe(sourcemaps.init())
        .pipe(sass().on('error', sass.logError))
        .pipe(sourcemaps.write())
        .pipe(dest('dist/pages/designer/styles'));
    done();
};
...
const buildScssProd = (done) => {
    ...
    src('src/pages/developer/styles/**/*.scss')
      .pipe(sass().on('error', sass.logError))
      .pipe(dest('./build/pages/developer/styles/'))
      .pipe(postcss())
      .pipe(rename({extname: '.min.css'}))
      .pipe(dest('./build/pages/developer/styles/'));
    src('src/pages/designer/styles/**/*.scss')
        .pipe(sass().on('error', sass.logError))
        .pipe(dest('./build/pages/designer/styles/'))
        .pipe(postcss())
        .pipe(rename({extname: '.min.css'}))
        .pipe(dest('./build/pages/designer/styles/'));
    done();
